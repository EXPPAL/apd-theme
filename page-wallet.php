...
$order_ids = apd_get_customer_orders( get_current_user_id() );
...
<section class="shop-carusel">
    ...
    <div class="carusel-wrapper">
        <?php if ($customer_orders->have_posts() && $order_ids ): ?>
...
<section class="history-table">
    ...
    $orders_by_date = new WP_Query($args);
    ?>
    ...
    <div class="table-wrapper">
        <table>
            ...
            <tbody>
            <?php if ($orders_by_date->have_posts()): ?>
                <?php while ($orders_by_date->have_posts()) : $orders_by_date->the_post(); ?>
                    <?php
                    $customer_order = wc_get_order(get_the_ID());
                    $order_meta = get_post_meta(get_the_ID());
    ...
    <div class="bottom-ribbon">
        <div class="pagination-holder"><?php
            ...
            $links = paginate_links(array(
                ...
                'format' => '?paged=%#%',
                'current' => max(1, get_query_var('paged')),
                'total' => $orders_by_date->max_num_pages,
            ));
            echo $links;
            ?></div>
    </div><!-- end .bottom-ribbon -->
