 ...
 * @extends \WC_Email
 */
class ApdAirDropOrderEmail extends WC_Email {
	...
	public function __construct() {
		...
		$this->id             = 'wc_apd_airdrop_order_completed';
		$this->customer_email = true;

		// this is the title in WooCommerce Email settings
		$this->title = 'AirDrop Order Completed';
	...
	public function process_shortcodes($html) {
		...
		foreach ($item as $product) {
			...
			if ( $this->is_airdrop_product($product) ) {
	...
	public static function log( $data ) {
		...
		if ( isset( $_GET['aops_log_to_screen'] ) && $_GET['aops_log_to_screen'] == 1 ) {
			...
			file_put_contents( $filename, date( "Y-m-d H:i:s" ) . "  airdrop | " . print_r( $data, 1 ) . "\r\n\r\n", FILE_APPEND );
		}
	}
}