 ...
 * @extends \WC_Email
 */
class ApdShopOrderEmail extends WC_Email {
	...
	public function __construct() {
		...
		$this->id             = 'wc_apd_shop_order_completed';
		$this->customer_email = true;

		// this is the title in WooCommerce Email settings
		$this->title = 'Audio Plugin Shop Order Completed';
		...
		$this->subject = 'Audio Plugin Shop Order Completed. {product_email_subject}';
	...
	public function trigger( $order_id ) {
		...
		if ( get_post_meta( $order_id, 'order_type', true ) == 'deal' || get_post_meta( $order_id, 'order_type', true ) == 'bigdeal' ) {
			return;
		}
		...
		if( $content ) {
			foreach ( $content as $item ) {
				self::set_order_email_template_content( $order_id, $item['body'], $item['product_id'] );
				$this->send( $this->get_recipient(), $item['subject'], $item['body'], $this->get_headers(), $this->get_attachments() );
			}
		}
	...
	public function process_shortcodes($html) {
		...
		foreach ($item as $product) {
			...
			if ( $this->is_shop_product($product) ) {
	...
	public function is_shop_product( $product ) {
		$is_shop_product = get_post_meta( $product['product_id'], '_product_type_single', true );
		return ( $is_shop_product == 'yes' );
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'subject'                 => array(
				'title'       => 'Subject',
				'type'        => 'text',
				'description' => sprintf( 'This controls the email subject line. '
								. 'Leave blank to use the default subject: <code>%s</code>.', $this->subject ),
				'placeholder' => '',
				'default'     => ''
			),
			'heading'                 => array(
				...
				'description' => sprintf( __( 'This controls the main heading contained within the email notification. '
								. 'Leave blank to use the default heading: <code>%s</code>.' ), $this->heading ),
				'placeholder' => '',
				'default'     => ''
			),
			'email_type'              => array(
			...
			'meta_field_email_subject' => array(
				'title' => 'Meta field where product-specific email header is stored',
				'type' => 'text',
				'description' => __('Product meta field used to obtain Email Subject. Default value is "apd_product_email_heading" '),
				'placeholder' => '',
				'default' => 'apd_product_email_heading'
			),
			'meta_field_email_template' => array(
				'title' => 'Meta field where product-specific email template is stored',
				'type' => 'text',
				'description' => __('Product meta field used to obtain Email Template. Default is "apd_product_email_template"'),
				'placeholder' => '',
				'default' => 'apd_product_email_template'
			),
		);
	}

	public static function set_order_email_template_content( $order_id, $content, $product_id ) {
		update_post_meta( $order_id, 'email_template_content_' . $product_id, $content );
	}
